...
describe('Heuristic Workspace Scanner', () => {
  ...
  it('scans a directory and detects Node.js structure', () => {
    ...
    const nodeSug = result.suggestions.find(s => s.fileName === 'node-commands.md');
  ...
  it('scans a directory and detects Python structure', () => {
    ...
    const pySug = result.suggestions.find(s => s.fileName === 'python-setup.md');
  ...
  it('writes suggestions successfully to target folder', () => {
    ...
    const expectedFilePath = path.join(tempTestDir, outDirName, writtenFileBase);
