    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">
            Review and explicitly authorize sensitive agent operations. Verify payload hashes before granting execution.
          </p>
        </div>
        <button
      ...
      {!loading && !error && approvals.length === 0 && (
        <div className="glass-panel p-12 rounded-2xl text-center border-dashed border-2 border-zinc-700/50 animate-in fade-in duration-700">
          ...
          <h3 className="text-lg font-medium text-emerald-400">No pending approvals</h3>
          <p className="text-zinc-500 mt-2 text-sm">
            All agent operations are currently running within their allowed safety boundaries.
          </p>
        </div>
      )}
      ...
      <div className="grid gap-6">
        {approvals.map((app) => (
          <div key={app.token} className="glass-panel p-6 rounded-2xl flex flex-col md:flex-row md:items-start justify-between border-l-4 border-l-neon-purple gap-6 animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex-1 space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-zinc-500 mt-1">
                  Target Platform: <span className="text-zinc-300 font-medium">{app.metadata?.platform || 'Unknown'}</span>
                </p>
              </div>

              <div className="bg-zinc-900/80 border border-white/5 p-3 rounded-lg flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-zinc-500 uppercase tracking-wider font-semibold">Cryptographic Payload Hash</span>
